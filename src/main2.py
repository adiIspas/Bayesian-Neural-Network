...
y_ph = tf.placeholder(tf.int32, [1])  # For the labels
...
inference.initialize(n_iter=STEPS, n_print=1, scale={y: N_TRAIN / BATCH_SIZE})
...
for _ in range(inference.n_iter):
    ...
    X_batch = tf.constant(X_batch, shape=[1, 2]).eval()
    Y_batch = next(train_labs_generator)
    Y_batch = tf.constant(Y_batch, shape=[1]).eval()
